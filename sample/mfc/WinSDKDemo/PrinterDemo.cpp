...
void PrinterDemo::loadDll()
{
	...
	if (hDllInst)
	{
		...
		PrintImageS = (pPrintImageW)GetProcAddress(hDllInst, "PrintImageW");
...
void PrinterDemo::PaperStatus(void* dlg)
{
	...
	if (rValue == 0)
	{
		if ((0x0C & status) > 0)
		{
			result = L"The printer status is Paper near end";
		}
		else
		{
			result = L"The printer status is Ready";
		}
	}
	else
	{
...
CString ParseStatus(int status)
{
	...
	{
		return L"Feed button has been pressed";
	}
	else if ((status & 0b100000) > 0)
	{
		return L"Printer is out of paper";
	}
	else if ((status & 0b1000000) > 0)
	{
		return L"Error condition";
	}
	else
	{
		return L"Error";
