...
namespace Printer
{
    ...
    internal class LKPrint : OqPrinter
    {        
        //Dll file name.
        ...
        public static extern Int32 _OpenPort(string PortName, Int32 BaudRate);
        
        [DllImport(SEWOODIR, SetLastError = true, EntryPoint = "ClosePort")]
        public static extern Int32 _ClosePort();
        
        [DllImport(SEWOODIR, SetLastError = true, EntryPoint = "PrintBitmap")]
        public static extern Int32 _PrintBitmap(string BitmapFile, Int32 Alignment, Int32 Options, Int32 Brightness, Int32 ImageMode);
        ...
        PrinterConfig mPrinterConfig;

        public bool Prepare(PrinterConfig printerConfig)
        {
            mPrinterConfig = printerConfig;

            switch(mPrinterConfig.connectionType)
            {
                case ConnectionType.USB:
                    return _OpenPort("USB", 38400) == LK_SUCCESS;
                case ConnectionType.IP:
                    return _OpenPort(printerConfig.detail, printerConfig.subValue) == LK_SUCCESS;
            }
        ...
        public bool Finish()
        {
            if(mPrinterConfig.connectionType != ConnectionType.DRIVER)
                return _ClosePort() == LK_SUCCESS;

            return true;
        }

        public bool PreparePrinter()
        {
            if (mPrinterConfig.connectionType == ConnectionType.DRIVER)
            {
                int lResult = LKPrint._OpenPort(mPrinterConfig.detail, 1);
                if (lResult != LK_SUCCESS)
                {
                    Logger.Error("Driver Open Error : " + lResult);
                }

                return lResult == LK_SUCCESS;
            }

            //1. 프린트 준비가 실패한 경우, 다시 초기화를 시도한다.
            //  - 파워 Off 후 다시 ON 한 경우
            if (PrintStart() != LK_SUCCESS && Prepare(mPrinterConfig))
                return PrintStart() == LK_SUCCESS;

            return true;
        }

        public bool CompletePrinter()
        {
            if (mPrinterConfig.connectionType == ConnectionType.DRIVER)
            {
                int lResult = LKPrint._ClosePort();
                if (lResult != LK_SUCCESS)
                {
                    Logger.Error("Driver Close Error : " + lResult);
                    return false;
                }
            }
        ...
        public bool PrintBitmap(string filePath)
        {
            ...
            bool result = _PrintBitmap(filePath, LK_ALIGNMENT_CENTER, LK_BITMAP_NORMAL, 5, LK_BITMAP_NO_DITHER) == LK_SUCCESS;
